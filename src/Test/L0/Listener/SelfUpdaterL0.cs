...
namespace GitHub.Runner.Common.Tests.Listener
{
    public sealed class SelfUpdaterL0
    {
        ...
        private async Task FetchLatestRunner()
        {
            ...
            _runnerServer.Setup(x => x.GetPackageAsync("agent", BuildConstants.RunnerPackage.PackageName, "12.999.0", true, It.IsAny<CancellationToken>()))
                         .Returns(Task.FromResult(new PackageMetadata() { Platform = BuildConstants.RunnerPackage.PackageName, Version = new PackageVersion("12.999.0"), DownloadUrl = _packageUrl }));
        ...
        public async void TestSelfUpdateAsync_DownloadRetry()
        {
            try
            {
                ...
                using (var hc = new TestHostContext(this))
                {
                    ...
                    _runnerServer.Setup(x => x.GetPackageAsync("agent", BuildConstants.RunnerPackage.PackageName, "12.999.0", true, It.IsAny<CancellationToken>()))
                             .Returns(Task.FromResult(new PackageMetadata() { Platform = BuildConstants.RunnerPackage.PackageName, Version = new PackageVersion("12.999.0"), DownloadUrl = $"https://github.com/actions/runner/notexists" }));
        ...
        public async void TestSelfUpdateAsync_ValidateHash()
        {
            try
            {
                ...
                using (var hc = new TestHostContext(this))
                {
                    ...
                    _runnerServer.Setup(x => x.GetPackageAsync("agent", BuildConstants.RunnerPackage.PackageName, "12.999.0", true, It.IsAny<CancellationToken>()))
                             .Returns(Task.FromResult(new PackageMetadata() { Platform = BuildConstants.RunnerPackage.PackageName, Version = new PackageVersion("12.999.0"), DownloadUrl = _packageUrl, HashValue = "bad_hash" }));
